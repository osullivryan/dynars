...
fn main() {
    ...
    for pid in [1, 2, 3, 4] {
        ...
        println!(
            "PART {pid} ({:?})  @ {}:{}",
            part.field("heading").map(|f| f.value()),
            part.file().display(),
            part.line()
        );
        if let Some(m) = &mat {
            println!(
                ...
                m.id().unwrap_or(0),
                m.name(),
                m.field("RO").map(|f| f.value()),
                m.field("E").map(|f| f.value())
            );
        }
        if let Some(s) = &sec {
            println!(
                "   section  -> id {} [{}]  elform={:?} nip={:?}",
                s.id().unwrap_or(0),
                s.name(),
                s.field("ELFORM").map(|f| f.value()),
                s.field("NIP").map(|f| f.value())
            );
        }
    }
}
